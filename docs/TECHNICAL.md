...
## 2.2. Неисправности и специализации

Соответствие задаётся на backend (`responsible_engineer` / `required_vehicle`) и дублируется на frontend только для подсказок диспетчеру.

| Неисправность | Специализация инженера | Спецтранспорт |
|---------------|------------------------|---------------|
| Подтекание топлива из дренажных колпачков | Инспектор целостности и герметичности | Топливозаправщик |
| Масляные пятна в районе редуктора | Инспектор целостности и герметичности | Маслораздаточная |
| Следы гидравлики на штоках амортизаторов шасси | Инспектор целостности и герметичности | — |
| Сколы и царапины на обтекателях/антеннах/фонарях | Авиатехник (диагностика/ремонт) | Подъёмник |
| Отслоение краски в зонах клёпки | Авиатехник (диагностика/ремонт) | Подъёмник |
| Отсутствие заглушек на приёмниках давления | Заправка (топливо/масло/кислород) | Подъёмник |
| Неравномерный износ протектора | Заправка (топливо/масло/кислород) | — |
| Порезы до корда | Заправка (топливо/масло/кислород) | — |
| Низкое давление в шинах | Заправка (топливо/масло/кислород) | — |
| Сбои индикации (лампа, предохранитель) | Обработка замечаний экипажа | КПА авионики |
| Ослабленный разъём | Обработка замечаний экипажа | КПА авионики |
| Регулировка привязных ремней | Обработка замечаний экипажа | — |
| Перегоревшая светосигнальная лампа | Обработка замечаний экипажа | — |
| Падение тяги/оборотов/температуры газов | Техник по двигателю | Бороскоп-тележка |
| Повышенная вибрация (дисбаланс) | Техник по двигателю | Бороскоп-тележка |
| Стружка в маслофильтре | Техник по двигателю | Бороскоп-тележка |
| Отказ/ложные показания РЛС | Инженер по авионике | КПА авионики |
| Потеря связи / искажение сигнала | Инженер по авионике | КПА авионики |
| Уход гироплатформы ИНС | Инженер по авионике | КПА авионики |
| Другое | Не категоризирован | — |

---

## 2.3. Где уместен ИИ в этой задаче

| Можно опираться на ИИ | Лучше без ИИ / только после ручной проверки |
|-----------------------|---------------------------------------------|
| Черновик UI, вёрстка, тексты подсказок | Контракт API (`/api/assign`, поля JWT, коды ошибок) |
| Демо-хаб, сценарии для жюри, документация | Маппинг неисправность → специализация / спецтранспорт (источник — backend enums) |
| Отрисовка тайлов, маркеры, подписи на карте | Алгоритм назначения и BFS на бэке |
| Формулировки ошибок для UI | Секреты, `.env`, прод-креды |
| README / техописание | «Починка» доменной логики наугад без прогона на стенде |

В коде точечно помечено комментариями `ИИ:` / `Без ИИ:` — как ориентир для проверки, а не как доля объёма работ.
